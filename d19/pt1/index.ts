...
class Scanner {
  ...
  setBeaconsFromMatrix3D(matrix: Matrix3D[]) {
...
const calcDistances = (scanner: Scanner) => {
  scanner.beacons.forEach(b1 => {
    scanner.beacons.forEach(b2 => {
      if (b1.x === b2.x && b1.y === b2.y && b1.z === b2.z) {
        return;
      }
      const distance = eDistance([b1.x, b1.y, b1.z], [b2.x, b2.y, b2.z]);
      b1.neighbors[b2.id] = distance;
    });
  });
}

const compareScanners = (mainScanner: Scanner, newScanner: Scanner): Set<string> => {
...
const parseCandidateSet = (set: Set<string>) => {
  const oneMatrix: Matrix3D[] = [];
  const twoMatrix: Matrix3D[] = [];
  set.forEach((val) => {
...
const calculateCentroid = (matrix: Matrix3D[]) => {
  let xSum = 0;
  let ySum = 0;
  let zSum = 0;
...
const center = (points: Matrix3D[], centroid: Matrix3D): Matrix3D[] => {
  const centeredPoints = points.map((point) => {
    return new Matrix(point).translateX(-centroid.x).translateY(-centroid.y).translateZ(-centroid.z).toObject();
  });

  return centeredPoints;
}

const rotateX = (points: Matrix3D[], rotation: number): Matrix3D[] => {
  return points.map(point => new Matrix(point).rotateX(rotation).toObject());
}

const rotateY = (points: Matrix3D[], rotation: number): Matrix3D[] => {
  return points.map(point => new Matrix(point).rotateY(rotation).toObject());
}

const rotateZ = (points: Matrix3D[], rotation: number): Matrix3D[] => {
  return points.map(point => new Matrix(point).rotateZ(rotation).toObject());
}

const rotate = (points: Matrix3D[], rotations: Matrix3D): Matrix3D[] => {
...
const findRotation = (point1: Matrix3D, point2: Matrix3D) => {
  let xRot = 0;
  let yRot = 0;
  let zRot = 0;
  let found = false;
  top:
  for (let i = 0; i < 4; i++) {
    const x = new Matrix({ ...point2 }).rotateX(i * 90);

    for (let j = 0; j < 4; j++) {
      const y = new Matrix({ ...x.toObject() }).rotateY(j * 90);

      for (let k = 0; k < 4; k++) {
        const z = new Matrix({ ...y.toObject() }).rotateZ(k * 90);
...
const translate = (matrix: Matrix3D[], translation: Matrix3D) => {
  return matrix.map((m) => {
    return new Matrix(m).translateX(translation.x).translateY(translation.y).translateZ(translation.z).toObject();
  });
};

const determineSign = (rotation: number) => {
  switch (rotation) {
    case 0: 
      return 1;
    case 1:
...
const determineScannerLocation = (p1: Matrix3D, p2: Matrix3D): Matrix3D => {
...
const findTotalRotation = (prevRot: Matrix3D, thisRot: Matrix3D) => {
  return { x: prevRot.x + thisRot.x, y: prevRot.y + thisRot.y, z: prevRot.z + thisRot.z };
}

const masterBeaconSet = new Set<string>();
...
const known: Scanner[] = [ masterScanner ];
let unknown: Scanner[] = [ ...scanners.slice(1) ];
